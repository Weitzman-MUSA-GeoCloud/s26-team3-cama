...
def extract_phl_real_estate_transfers(request):

    logging.info("Extracting Real Estate Transfers...")

    try:
        start_year = int(os.environ["START_YEAR"])
        start_date = datetime.date(start_year, 1, 1).isoformat()
        bucket_name = os.environ["BUCKET_NAME"]
        blob_name = os.environ["BLOB_REAL_ESTATE"]
    except (KeyError, ValueError) as e:
        logging.error(f"Configuration error {e}")
        return ("Server misconfiguration", 500)

    query_realest = f"SELECT * FROM RTT_SUMMARY WHERE display_date >= '{start_date}'"
    URL_REALESTATE = "https://phl.carto.com/api/v2/sql"

    try:
        data = data_to_parquet(URL_REALESTATE, query_realest)
    ...
    return ("Successfully imported Real Estate Transfers.", 200)
